...
static lock_t *spls_find_lock(app_data_t *app, const char *name)
{
...
static void spls_add_link(lock_t *lock, pn_link_t *link)
{
...
static bool spls_link_attached(app_data_t *app, pn_link_t *link, const char *name)
{
...
static pn_link_t *spls_get_next_link(lock_t *lock)
{
...
static void spls_remove_link(lock_t *lock, pn_link_t *link)
{
...
static pn_link_t *spls_link_detached(app_data_t *app, pn_link_t *link, const char *name)
{
    lock_t *lock = spls_find_lock(app, name);
...
static bool handle(app_data_t* app, pn_event_t* event)
{
    ...
    case PN_LINK_REMOTE_OPEN: {
        ...
        bool open = spls_link_attached(app, link, addr);
        if (open) {
    ...
    case PN_LINK_REMOTE_DETACH: {
        ...
        if (addr) {
            pn_link_t *next_link = spls_link_detached(app, link, addr);
            if (next_link) {
